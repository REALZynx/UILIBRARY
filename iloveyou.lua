...
local Library do
	...
	local Components = { } do
		...
		Components.ToggleDropdown = function(Data)
			...
			-- SetValue: silent set tanpa trigger callback, mirip Linoria
			function Dropdown:SetValue(Values)
				if Data.Multi then
					...
					for _, OptionName in Values do
						...
						OptionData.Selected = true
						OptionData:Toggle("Active")
						TableInsert(self.Value, OptionName)
					end
					...
					local OptionData = self.Options[Values]
					if not OptionData then
						for Index, Value in self.Options do
							Value.Selected = false
							Value:Toggle("Inactive")
	...
	Library.Notification = function(self, Text, Description, Duration, Buttons)
		...
		local Items = { } do
			...
			if HasButtons then
				...
				for Index, ButtonData in ipairs(Buttons) do
					local BtnLabel = ButtonData[1] or ButtonData.Text or "OK"
					local BtnCallback = ButtonData[2] or ButtonData.Callback or function() end
					...
					local Btn = Instances:Create("TextButton", {
						...
						AutoButtonColor = false,
						LayoutOrder = Index
					})  Btn:AddToTheme({BackgroundColor3 = "Inline"})
					...
					Btn.Instance.MouseEnter:Connect(function()
						Tween:Create(Btn.Instance, TweenInfo.new(0.15), {BackgroundColor3 = FromRGB(60, 52, 62)})
					end)
	...
	Library.KeybindList = function(self)
		...
		local Items = { } do
			...
			-- Header row: title kiri, label "Mobile" kanan
			...
			Items["MobileLabel"] = Instances:Create("TextLabel", {
				Parent = Items["Header"].Instance,
				Name = "\0",
				FontFace = Library.Font,
				TextColor3 = FromRGB(130, 120, 140),
				BorderColor3 = FromRGB(0, 0, 0),
				Text = "Mobile",
				BackgroundTransparency = 1,
		...
		function KeybindList:Add(Key, Name, Mode, Callback)
			-- Row container
			...
			-- Label kiri: Key - Name (Mode)
			local KeyLabel = Instances:Create("TextLabel", {
				Parent = Row.Instance,
				Name = "\0",
				FontFace = Library.Font,
				TextColor3 = FromRGB(210, 210, 215),
				BorderColor3 = FromRGB(0, 0, 0),
				Text = Key .. "  " .. Name,
				BackgroundTransparency = 1,
			...
			local ModeBadge = Instances:Create("TextLabel", {
				...
				Name = "\0",
				FontFace = Library.Font,
				TextColor3 = FromRGB(160, 150, 170),
				BorderColor3 = FromRGB(0, 0, 0),
				Text = Mode,
				BackgroundTransparency = 0.6,
				BackgroundColor3 = FromRGB(40, 35, 45),
				BorderSizePixel = 0,
				AutomaticSize = Enum.AutomaticSize.X,
				...
				TextSize = 11,
				TextXAlignment = Enum.TextXAlignment.Center,
				Visible = false -- disembunyikan, info Mode ada di tooltip
			})
			Instances:Create("UICorner", { Parent = ModeBadge.Instance, CornerRadius = UDimNew(0, 3) })
			Instances:Create("UIPadding", {
				Parent = ModeBadge.Instance,
				PaddingLeft = UDimNew(0, 4),
				PaddingRight = UDimNew(0, 4)
			})

			-- Toggle mobile button di kanan (pill-style)
			local ToggleEnabled = false

			local ToggleBtn = Instances:Create("Frame", {
				Parent = Row.Instance,
				Name = "\0",
				...
				BorderSizePixel = 0,
				BorderColor3 = FromRGB(0, 0, 0),
				BackgroundColor3 = FromRGB(44, 38, 50)
			})  ToggleBtn:AddToTheme({BackgroundColor3 = "Inline"})
			...
			local Knob = Instances:Create("Frame", {
				...
				BorderSizePixel = 0,
				BorderColor3 = FromRGB(0, 0, 0),
				BackgroundColor3 = FromRGB(130, 120, 140)
			})
			...
			-- Clickable overlay di atas toggle
			local ToggleClick = Instances:Create("TextButton", {
				Parent = ToggleBtn.Instance,
				Name = "\0",
				Size = UDim2New(1, 0, 1, 0),
				BackgroundTransparency = 1,
			...
			local function SetToggle(Bool, Silent)
				ToggleEnabled = Bool
				if Bool then
					Tween:Create(ToggleBtn.Instance, TweenInfo.new(0.2), {BackgroundColor3 = Library.Theme.Accent})
					Tween:Create(Knob.Instance, TweenInfo.new(0.2), {
						Position = UDim2New(1, -19, 0.5, 0),
						BackgroundColor3 = FromRGB(255, 255, 255)
					})
					KeyLabel:ChangeItemTheme({TextColor3 = "Accent"})
					KeyLabel:Tween(nil, {TextColor3 = Library.Theme.Accent})
				else
					...
					KeyLabel:ChangeItemTheme({TextColor3 = "Text"})
					KeyLabel:Tween(nil, {TextColor3 = Library.Theme.Text})
				end
			...
			ToggleClick.Instance.MouseButton1Click:Connect(function()
				SetToggle(not ToggleEnabled)
			end)

			function NewKey:Set(NewKeyName, NewName, NewMode)
				KeyLabel.Instance.Text = NewKeyName .. "  " .. NewName
			end
	...
	Library.Watermark = function(self, Name)
		...
		local Items = { } do
			-- Container utama: AutomaticSize XY supaya ngikut konten
			...
			-- Border stroke halus ikut theme
			Instances:Create("UIStroke", {
				Parent = Items["Watermark"].Instance,
				Name = "\0",
				Color = FromRGB(41, 37, 45),
			...
			-- Padding: beri ruang di semua sisi
			Instances:Create("UIPadding", {
				Parent = Items["Watermark"].Instance,
				Name = "\0",
			...
			-- Text label ukuran otomatis XY, satu baris
			Items["Text"] = Instances:Create("TextLabel", {
				Parent = Items["Watermark"].Instance,
				Name = "\0",
				FontFace = Library.Font,
		...
		Library:Connect(RunService.RenderStepped, function()
			...
			-- Warna FPS: hijau >= 60, kuning 30-59, merah < 30
			local FpsColor = FramesPerSecond >= 60 and "#7dff7d"
				or FramesPerSecond >= 30 and "#ffd97d"
				or "#ff7d7d"

			-- Warna Ping: hijau < 80, kuning 80-149, merah >= 150
			local PingColor = Ping < 80 and "#7dff7d"
				or Ping < 150 and "#ffd97d"
				or "#ff7d7d"

			Items["Text"].Instance.Text = StringFormat(
				"<b>%s</b>  <font color='#aaaaaa'>fps</font> <font color='%s'>%d</font>  <font color='#aaaaaa'>ping</font> <font color='%s'>%d</font><font color='#aaaaaa'>ms</font>",
				Name, FpsColor, FramesPerSecond, PingColor, Ping
			)
		end)
	...
	Library.Page = function(self, Properties)
		...
		local Items = { } do
			...
			if not Page.IsKeyPage then
				-- Wrapper vertikal: FullColumn (OneSection) di atas, Columns biasa di bawah
				Items["PageInner"] = Instances:Create("Frame", {
				...
				Instances:Create("UIListLayout", {
					Parent = Items["PageInner"].Instance,
					Name = "\0",
				...
				-- Full-width scrolling column khusus OneSection
				Items["FullColumn"] = Instances:Create("ScrollingFrame", {
					Parent = Items["PageInner"].Instance,
					Name = "\0",
				...
				Instances:Create("UIListLayout", {
					Parent = Items["FullColumn"].Instance,
					Name = "\0",
					Padding = UDimNew(0, 8),
					SortOrder = Enum.SortOrder.LayoutOrder
				})

				Page.FullColumnData = Items["FullColumn"]

				-- Frame wrapper 2-column biasa
	...
	Library.Pages.OneSection = function(self, Properties)
		Properties = Properties or { }
		...
		local Items = { } do
			Items["Section"] = Instances:Create("Frame", {
				Parent = Section.Page.FullColumnData.Instance,
				Name = "\0",
				BorderSizePixel = 0,
				Size = UDim2New(1, 0, 0, 45),
				BorderColor3 = FromRGB(0, 0, 0),
	...
	Library.Sections.Dropdown = function(self, Properties)
		...
		-- SetValue: silent set tanpa callback, mirip Linoria
		function Dropdown:SetValue(Values)
			NewDropdown:SetValue(Values)
		end
